...
try {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if ($result->num_rows > 0) {
            ...
            $ps = $conn->prepare("INSERT INTO pending_user (username, email, `password`, role_id, person_type_id, verification_code, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
            if ($ps === false) {
                die("Prepare failed: " . $conn->error);
            }

            $ps->bind_param("sssissss", $name, $email, $hash_password, $role_id, $person_type_id, $verificationCode, $create_at, $update_at);
            $ps->execute();
